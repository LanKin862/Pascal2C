...
+ 类型检查：实现了对：
  + 变量声明与使用是否匹配——如果存在未声明的变量使用，则进行报错，并继续尝试转换
  + 赋值语句左右类型是否匹配——如果存在左右类型不同的情况，则进行报错，并继续尝试转换
  + 函数返回类型检查——如果存在函数最后返回值与函数类型不符的情况，则进行报错，并继续尝试转换
  + 类型转换是否合法（部分隐式类型转换）
+ 表达式：
  + 表达式的最终结果的类型判断
+ 数组：
  + 数组索引类型检查——如果出现小数，则报错并尝试自动向下取整
  + 数组维数检查（多维数组）
...
+ 函数/过程调用：
  + 函数原型声明
  + 返回值是否正确使用——若未使用该返回值，则会警告忽略了函数的返回值。并继续尝试转换
+ 语句：
  + 支持语句：`if`，`else`，`break`，`for`，`while`
  + 变量作用域检查
  + 赋值语句类型兼容性——如果两边操作不兼容/将过程加入赋值操作中，则报错，并继续转换
  + 条件语句条件表达式须是布尔类型
  + 跳转语句（如break、continue）是否在合法上下文中【未做】
+ 运算：
  + 支持运算：$\div$ $mod$  $+$ $-$ $\times$ ~ $\&$ $|$   
